...
private class Row(val placeables: List<Placeable>, val horizontalGapPx: Int) {
    val width by lazy(LazyThreadSafetyMode.NONE) { placeables.sumOf { it.width } + (placeables.size - 1) * horizontalGapPx }
    val height by lazy(LazyThreadSafetyMode.NONE) { placeables.maxOfOrNull { it.height } ?: 0 }
}
